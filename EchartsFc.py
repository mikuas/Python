...
class Echarts:
    ...
    @staticmethod
    def getPieChart(name=None, render=False, **kwargs):
        """
        :param name: Name: Str
        :param kwargs:
        data: List -> (): [(key, value), (key, value), ...]
        color: key -> color: List
        mainTitle: Title: Str
        :return: Pie()
        """
        pie = (
            Pie()
            .add(series_name=name, data_pair=kwargs['data'])
            .set_colors(kwargs['color'])
            .set_global_opts(title_opts=TitleOpts(title=kwargs['mainTitle']))
            .set_series_opts(label_opts=LabelOpts(formatter="{b}: {c}"))
        )
        if render:
            pie.render(kwargs['HTML_Name'])
    ...
    def getTimeBars(text_color=None, **kwargs):
        ...
        for i in range(kwargs['TimeLineNumber']):
            result = Echarts().getBars(
                ...
                y_data=kwargs['y_data'][i],
                title=kwargs['title'],
                position=kwargs['position'],
                main_title=kwargs['main_title'],
                HTML_Name=kwargs['HTML_Name'][1],
                PT=kwargs['PT'],
                angle=kwargs['angle'],
    ...
    @staticmethod
    def getTimePieChart(lines, **kwargs):
        """
        :param lines: line = data length
        :param kwargs:
        data: List -> (): [[(key, value), (key, value), ...], ...]
        color: key -> color: List
        mainTitle: Title: Str
        timeline: Object -> Timeline()
        lineTitle: Title: List, lineTitle length = data length
        time: interval time
        HTML_Name: Str
        play, For: Bool
        :return:
    ...
    @staticmethod
    def readFileTimeBarCharts(text_color=None, center='center', width=None, height=None, reverse=False, **kwargs):
        """
        :param text_color: text Color
        :param center: align
        :param width: Timeline_width
