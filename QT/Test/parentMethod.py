...
class SystemCtl:
    ...
    def createUser(self, userName: str, password: str | int, manager=False) -> 'SystemCtl':
        """
        创建用户
        :param userName: 用户名
        :param password: 密码
...
class FileControl:
    ...
    def getFilePackagePath(self, fileName) -> str:
        """
        获取打包后绝对路径
        :param fileName: 文件名
        :return: 文件绝对路径
    ...
    def getFileAbsolutePath(self, fileName) -> str:
        """
        获取文件的绝对路径
        :param fileName: 文件名
        :return: 文件的绝对路径
...
class Regedit:
    ...
    def addFileLeftKeyClick(self, name: str, path: str, iconPath=None, args=False) -> 'Regedit':
        """
        添加右键点击文件选项
        :param name: 名称
        :param path: 要执行的文件路径
